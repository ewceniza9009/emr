...
const UPDATE_FORM = gql`
  mutation UpdateForm($input: QuestionnaireInput!) {
...
function mapToSurveyType(legacyType: string | undefined | null) {
  ...
  if (t.includes("SCALE") || t.includes("RATING") || t.includes("1")) return "rating";
...
export default function FormDesignerPage() {
  ...
  const initialJson = useMemo(() => {
    ...
    if (form.questions?.length > 0) {
      console.log("Synthesizing from legacy questions:", form.questions.length);
      ...
      return JSON.stringify({ pages: [{ name: "legacy_import", elements }] });
    }
    return null;
  }, [form]);
