      ...
      <div className="bg-[var(--card-bg)] rounded-[2.5rem] border border-[var(--card-border)] p-4 flex flex-col gap-4 overflow-hidden h-full">
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pr-1 pb-10 custom-scrollbar">
          {groupedList.map((group: any) => {
            ...
            return (
              <div key={group.patientId} className="bg-[var(--input-bg)]/40 border border-[var(--card-border)] rounded-2xl p-3 animate-in fade-in space-y-2">
                {/* Patient Header */}
                <div 
                ...
                >
                  <div className="flex items-center gap-2">
                    <ChevronRight className={`w-3.5 h-3.5 transition-transform duration-200 ${isExpanded ? 'rotate-90 text-[var(--primary)]' : 'text-[var(--text-muted)] group-hover/hdr:text-[var(--text-primary)]'}`} />
                    <div>
                      <h4 className="text-xs font-black uppercase text-[var(--text-primary)] group-hover/hdr:text-[var(--primary)] transition-colors">{group.firstName} {group.lastName}</h4>
                      <span className="text-[8px] font-black text-[var(--text-muted)] uppercase tracking-wider">MRN: {group.mrn}</span>
                    </div>
                  </div>
                  <span className="px-2 py-0.5 rounded-full text-[8px] font-black bg-[var(--primary)]/10 text-[var(--primary)] uppercase tracking-tighter shrink-0">
                    {group.notes.length} {group.notes.length === 1 ? 'visit' : 'visits'}
                  </span>
                </div>
                    ...
                    {group.notes.map((n: any) => (
                      ...
                      >
                        <div className="space-y-1">
                          ...
                          <span className="text-[9px] font-bold text-[var(--text-secondary)] uppercase">Clinical Note</span>
                        </div>
                        <ChevronRight className={`w-3.5 h-3.5 transition-transform group-hover/note:translate-x-0.5 ${
                          selectedId === n.appointmentId ? 'text-[var(--primary)]' : 'text-[var(--text-muted)]'
                        }`} />
                      </div>
                    ))}
                  </div>
