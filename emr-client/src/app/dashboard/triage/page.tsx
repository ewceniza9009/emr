...
import TriageFilterPopover, {
  TriageFilters,
} from "@/components/TriageFilterPopover";
...
export default function TriageDashboard() {
    ...
    connection
      ...
      .then(() => {
        ...
        connection.on("ReceiveAlert", (patientId, alertMessage) => {
          console.log(
            "Received alert, refetching triage list",
            patientId,
            alertMessage,
          );
          refetch();
  ...
  const totalCount = data?.triageWorklist?.totalCount || 0;
  const facilityOutreach = data?.facilityOutreach || [];

  const { sortField, sortOrder, handleSort, sortedItems: sortedTriageItems } = useSort<any>(triageItems, {
    customAccessors: {
      name: (item: any) => `${item.firstName} ${item.lastName}`,
      latestPainScore: (item: any) => Number(item.latestPainScore) || 0,
    },
  });
      ...
      await claimTriage({
        variables: {
          input: {
            patientId,
            triageNote:
              "[Triage: Critical] Triage Claimed. Acknowledged by Navigator.",
          },
        },
      });
      refetch();
    ...
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-sm font-bold text-[var(--text-primary)] uppercase tracking-tight">
            Clinical Priority Overview
          </h1>
          <p className="text-sm text-[var(--text-secondary)]">
            Managing patient urgency based on symptom burden and clinical
            alerts.
          </p>
        </div>
        <div className="flex gap-3">
          <div className="px-4 py-2 rounded-xl bg-red-500/10 border border-red-500/30 text-red-600">
            <div className="text-[9px] uppercase font-black tracking-widest leading-none mb-1">
              Critical Symptom Burden
            </div>
      ...
      <div className="bg-[var(--card-bg)] rounded-[2rem] border border-[var(--card-border)] p-4 flex items-center gap-4">
        <div className="flex-1 relative">
          <Search
            className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--text-muted)] transition-all ${loading ? "animate-pulse text-amber-500" : ""}`}
          />
          <input
            type="text"
            value={searchQuery}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="glass-morphism rounded-2xl border border-[var(--card-border)] relative">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-[var(--input-bg)] text-[var(--text-muted)] text-[10px] uppercase font-bold tracking-widest border-b border-[var(--card-border)] select-none">
                    <th 
                      onClick={() => handleSort("name")}
                      className="px-4 py-4 cursor-pointer hover:text-[var(--text-primary)] transition-colors"
                    >
                    ...
                    <th 
                      onClick={() => handleSort("latestPainScore")}
                      className="px-4 py-4 text-center cursor-pointer hover:text-[var(--text-primary)] transition-colors"
                    >
                      <div className="flex items-center justify-center gap-1">
                        Symptom Burden
                        {sortField === "latestPainScore" ? (
                          sortOrder === "asc" ? <ArrowUp className="w-3.5 h-3.5" /> : <ArrowDown className="w-3.5 h-3.5" />
                        ) : (
                          <ArrowUpDown className="w-3 h-3 opacity-40" />
                        )}
                    ...
                    <th 
                      onClick={() => handleSort("advanceDirectiveType")}
                      className="px-4 py-4 text-center cursor-pointer hover:text-[var(--text-primary)] transition-colors"
                    >
                      <div className="flex items-center justify-center gap-1">
                        Advance Directive
                        {sortField === "advanceDirectiveType" ? (
                          sortOrder === "asc" ? <ArrowUp className="w-3.5 h-3.5" /> : <ArrowDown className="w-3.5 h-3.5" />
                        ) : (
                          <ArrowUpDown className="w-3 h-3 opacity-40" />
                        )}
                  ...
                  {sortedTriageItems.map((p: any) => (
                    ...
                    >
                      <td
                        className="px-4 py-3"
                        onClick={() =>
                          router.push(`/dashboard/patients/${p.patientId}`)
                        }
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-2 h-2 rounded-full ${p.isAlert ? "bg-red-500 animate-pulse shadow-[0_0_10px_rgba(239,68,68,0.4)]" : "bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.2)]"}`}
                          />
                          <div>
                            <p className="text-xs font-bold text-[var(--text-primary)] tracking-tight group-hover:text-[var(--primary)] transition-colors">
                      ...
                      >
                        <div
                          className={`inline-flex items-center justify-center px-3 py-1 rounded-xl text-[10px] font-black tracking-tight border shadow-sm
                      ...
                      >
                        <div className="max-w-[180px] space-y-1">
                          <p className="text-[10px] text-[var(--text-primary)] font-medium line-clamp-1 italic opacity-80">
                            {p.triageNote ||
                              (p.isAlert
                                ? "Patient reporting breakthrough pain. Requires symptom review."
                                : "Clinical status remains stable based on last encounter.")}
                          </p>
                      ...
                      >
                        <span
                          className={`px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-[0.15em] border shadow-sm
        ...
        <div className="space-y-4">
              ...
              {facilityOutreach.map((f: any) => (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="text-[var(--text-primary)] font-bold text-sm group-hover:text-emerald-400 transition-colors">
                      {f.name}
                    </h3>
                  ...
                  <div className="flex gap-3">
                    <div className="text-[10px] text-[var(--text-muted)] uppercase tracking-wider font-bold">
                      {f.patientCount} Patients
                    </div>
                    {f.crisisCount > 0 && (
                      <div className="text-[10px] text-red-500 uppercase tracking-wider font-bold">
                        {f.crisisCount} In Crisis
                      </div>
                    )}
                  </div>
                </div>
              ...
              {facilityOutreach.length === 0 && (
                <div className="py-8 text-center">
                  <p className="text-[var(--text-muted)] text-xs italic">
                    No active facility outreach programs found.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="glass-morphism rounded-2xl p-4 border border-[var(--card-border)] shadow-lg">
            ...
            <p className="text-[var(--text-muted)] text-[10px] mb-4 leading-relaxed italic">
              The following patients are in high-risk groups but lack a
              documented Advance Directive.
            </p>
            <div className="space-y-3">
              {triageItems
                .filter((p: any) => p.advanceDirectiveType === "None")
                .slice(0, 4)
                .map((p: any) => (
                  <div
                    key={p.patientId}
                    className="flex items-center justify-between text-sm group p-1 hover:bg-[var(--primary)]/5 rounded-lg transition-all"
                  >
                    <span
                      onClick={() =>
                        router.push(`/dashboard/patients/${p.patientId}`)
              ...
              {triageItems.filter((p: any) => p.advanceDirectiveType === "None")
                .length === 0 && (
                <p className="text-[10px] text-emerald-500 font-bold uppercase tracking-widest text-center py-4">
                  All high-risk records validated.
                </p>
              )}
            </div>
          </div>
