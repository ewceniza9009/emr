...
const GET_VISIT_SUMMARY = gql`
  query GetVisitSummary($patientId: UUID!, $appointmentId: UUID!) {
    ...
    encounters: encountersByPatient(
      patientId: $patientId
...
const GET_ASSESSMENT_RESPONSES = gql`
  query GetAssessmentResponses($encounterId: UUID!) {
    assessmentResponses: assessmentResponsesByEncounter(
    ...
    spiritualAssessments: spiritualAssessmentsByEncounter(
      encounterId: $encounterId
    ) {
...
export default function VisitSummaryDrawer({
  ...
  let esas = data?.esasHistory?.find(
    (e: any) =>
      e.encounterId?.toLowerCase() === encounter?.encounterId?.toLowerCase(),
  );
  ...
  } = useQuery(GET_ASSESSMENT_RESPONSES, {
    variables: { encounterId: encounter?.encounterId },
    skip: !encounter?.encounterId,
  });

  if (!esas && assessmentData?.assessmentResponses) {
    const esasResponse = assessmentData.assessmentResponses.find((r: any) =>
      r.questionnaire?.name?.toUpperCase().includes("ESAS"),
    );

    if (esasResponse) {
      try {
        const answers = JSON.parse(esasResponse.answersJson || "{}");
        const qs = esasResponse.questionnaire?.questions || [];

        const getVal = (label: string) => {
          const q = qs.find((q: any) =>
            q.text?.toLowerCase().includes(label.toLowerCase()),
          );
          if (!q) return 0;
          const val = answers[q.questionId] ?? answers[String(q.order)] ?? 0;
          return typeof val === "number" ? val : parseFloat(String(val)) || 0;
        };

        esas = {
          pain: getVal("Pain"),
          tiredness: getVal("Tiredness"),
          drowsiness: getVal("Drowsiness"),
          nausea: getVal("Nausea"),
          lackOfAppetite: getVal("Appetite"),
          shortnessOfBreath: getVal("Breath"),
          depression: getVal("Depression"),
          anxiety: getVal("Anxiety"),
          wellbeing: getVal("Wellbeing"),
        };
      } catch (e) {
        console.error("Failed to parse ESAS fallback data", e);
      }
    }
  }
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/clinical/export/encounter/${appointmentId}`,
        {
          headers: {
      ...
      alert({
        ...
        message:
          err.message ||
          "An error occurred while generating the clinical summary PDF. Please check connection and try again.",
        type: "danger",
      });
    } finally {
          ...
          <div className="relative w-full max-w-2xl bg-[var(--sidebar-bg)] border-l border-[var(--card-border)] shadow-[-50px_0_150px_rgba(0,0,0,0.3)] h-full flex flex-col animate-in slide-in-from-right duration-500">
            ...
            <div className="h-20 w-full flex items-center justify-between px-8 bg-[var(--sidebar-bg)] border-b border-[var(--card-border)] shrink-0 backdrop-blur-md">
              <div className="flex items-center gap-6">
                ...
                <div>
                  <h2 className="text-sm font-bold text-[var(--text-primary)] tracking-tight leading-none uppercase">
                    Visit Summary
                  </h2>
                  <p className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em] mt-2 flex items-center gap-2">
                    ...
                    {data?.appointment
                      ? new Date(
                          data.appointment.scheduledStart,
              ...
              <div className="flex items-center gap-3">
                {(() => {
                  ...
                  return (
                    (status === "LIVE" || status?.includes("PROGRESS")) && (
              ...
              {loading ? (
                <div className="flex flex-col items-center justify-center py-20 space-y-6">
                  ...
                  <p className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] animate-pulse">
                    Retrieving Encrypted Clinical Record...
                  </p>
                </div>
              ) : error ? (
                <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest mt-2">
                      {error.message ||
                        "Unauthorized or network failure detected."}
                    </p>
                  </div>
                </div>
              ) : encounter ? (
                <>
                  ...
                  <div className="p-8 rounded-[2.5rem] bg-[var(--input-bg)] border border-[var(--card-border)] relative overflow-hidden group space-y-5">
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-[9px] font-black text-[var(--primary)] uppercase tracking-widest px-2 py-0.5 rounded-md bg-[var(--primary)]/10">
                            Attending Practitioner
                          </span>
                        </div>
                        <h3 className="text-sm font-bold text-[var(--text-primary)] uppercase truncate">
                        ...
                        <p className="text-xs font-medium text-[var(--text-muted)] italic">
                          {encounter.practitioner?.position}
                        </p>
                      </div>
                      <div className="hidden md:block text-right shrink-0">
                        <p className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-widest mb-1.5">
                          Encounter ID
                        </p>
                        <p className="text-[11px] font-mono font-bold text-[var(--text-primary)] opacity-50">
                          {encounter.encounterId.slice(0, 13).toUpperCase()}
                        </p>
                      </div>
                    </div>
                    ...
                    {(data?.appointment?.visitType ||
                      ...
                      <div className="flex items-center gap-2 pt-1 border-t border-[var(--card-border)]">
                        <p className="text-[8px] font-black text-[var(--text-muted)] uppercase tracking-[0.3em] mr-1">
                          Visit Context
                        </p>
                        {data.appointment.visitType && (
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {[
                        {
                          label: "Heart Rate",
                          value: encounter.vitalSigns?.[0]?.heartRate,
                          unit: "BPM",
                          icon: Heart,
                          color: "text-rose-500",
                        ...
                        {
                          label: "BP",
                          value: encounter.vitalSigns?.[0]
                            ?.bloodPressureSystolic
                            ? `${encounter.vitalSigns[0].bloodPressureSystolic}/${encounter.vitalSigns[0].bloodPressureDiastolic}`
                            : null,
                          unit: "mmHg",
                          icon: Activity,
                          color: "text-blue-500",
                        ...
                        {
                          label: "Body Temp",
                          value: encounter.vitalSigns?.[0]?.temperature,
                          unit: " °F",
                          icon: Thermometer,
                        ...
                        {
                          label: "Oxygen Sat",
                          value: encounter.vitalSigns?.[0]?.oxygenSaturation,
                          unit: "% SpO2",
                          icon: Wind,
                        ...
                        <div
                          key={i}
                          className="p-5 rounded-3xl bg-[var(--input-bg)] border border-[var(--card-border)] hover:border-[var(--primary)]/40 transition-all duration-300 group/v"
                        >
                          <div className="flex items-center gap-2 mb-3">
                            <v.icon
                              className={`w-3.5 h-3.5 ${v.color} group-hover/v:scale-110 transition-transform`}
                            />
                        ...
                        {[
                          ...
                          <div key={i} className="space-y-2.5">
                            <div className="flex justify-between items-center px-1">
                              ...
                              <span
                                className={`text-[10px] font-black ${s.value > 7 ? "text-rose-500" : s.value > 3 ? "text-amber-500" : "text-emerald-500"}`}
                              >
                                {s.value}/10
                              </span>
                            </div>
                  ...
                  <div className="space-y-6">
                    ...
                    {assessmentLoading && (
                      <div className="p-10 text-center border border-dashed border-[var(--card-border)] rounded-[2rem] bg-[var(--input-bg)]/50 animate-pulse">
                        <p className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest">
                          Scanning Encounter Assessments...
                        </p>
                      </div>
                    )}
                    ...
                    {!assessmentLoading &&
                      ...
                      assessmentData?.spiritualAssessments?.length === 0 && (
                        <div className="p-10 text-center border border-dashed border-[var(--card-border)] rounded-[2rem] bg-[var(--input-bg)]/50">
                          <p className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest opacity-40 italic">
                            No formal assessments documented for this encounter.
                          </p>
                        </div>
                      )}
                      ...
                      (assessmentData.assessmentResponses?.length > 0 ||
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {(() => {
                            ...
                            // Sort by completion date descending to get latest first
                            const sorted = [...(assessmentData.assessmentResponses || [])].sort((a, b) => 
                              new Date(b.completedAt).getTime() - new Date(a.completedAt).getTime()
                            );

                            sorted.forEach((res: any) => {
                              const qId = res.questionnaire?.questionnaireId;
                              if (!seenIds.has(qId)) {
                                uniqueResponses.push(res);
                                seenIds.add(qId);
                              }
                            });
                          ...
                          {assessmentData.spiritualAssessments?.map(
                            (s: any) => (
                              ...
                              >
                                ...
                                <h5 className="text-[10px] font-black text-[var(--text-primary)] uppercase tracking-widest mb-3">
                                  Spiritual Assessment (FICA)
                                </h5>
                                <div className="grid grid-cols-2 gap-4">
                                  <div>
                                    <p className="text-[7px] font-black text-[var(--text-muted)] uppercase tracking-widest">
                                      Faith
                                    </p>
                                    <p className="text-[9px] font-bold text-[var(--text-primary)] truncate">
                                      {s.faith || "N/A"}
                                    </p>
                                  </div>
                                  <div>
                                ...
                                <div className="mt-4 pt-3 border-t border-[var(--card-border)]">
                                  <span className="text-[8px] font-black text-indigo-400 uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">
                                    Full History →
                                  </span>
                                </div>
                              </button>
                          ...
                          >
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <p className="text-[9px] text-[var(--text-muted)] font-medium mt-1 line-clamp-1">
                                  {d.notes ||
                                    "Documented on " +
                                      new Date(
                                        d.effectiveDate,
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <span className="px-2.5 py-1 rounded-lg bg-emerald-500 text-white text-[8px] font-black uppercase tracking-widest">
                                Active
                              </span>
                            </div>
                          </button>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      {encounter.clinicalNotes?.map((note: any, i: number) => (
                        <div
                          key={i}
                          className="relative pl-8 border-l border-[var(--card-border)] py-1"
                        >
                  ...
                  {/* Certification */}
                  <div className="p-6 rounded-[2rem] bg-emerald-500/5 border border-emerald-500/10 flex flex-col md:flex-row items-center gap-6 relative overflow-hidden">
                    ...
                    <div>
                      <p className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.3em] mb-2">
                        Electronic certification
                      </p>
                      <p className="text-sm text-[var(--text-muted)] leading-relaxed italic font-medium">
                        I hereby certify that the clinical services described
                        herein were personally rendered by me or under my direct
                        supervision. This clinical record was electronically
                        signed by{" "}
                        <span className="text-[var(--text-primary)] font-bold not-italic">
                          {encounter.practitioner?.firstName}{" "}
                          {encounter.practitioner?.lastName}
                        </span>{" "}
                        on{" "}
                        {new Date(
                          encounter.dischargedAt || encounter.encounterDate,
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest mt-2">
                      No clinical documentation could be retrieved for this
                      appointment ID.
                    </p>
                  </div>
                </div>
