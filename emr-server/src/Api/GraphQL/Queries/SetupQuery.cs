...
public class SetupQuery
{
    ...
    )
    {
        ...
        if (patient == null) return new List<PractitionerResourceDto>();
        ...
        var activeCaseCounts = await context.CareNavigationCases
            .Where(c => c.Status == Domain.Enums.CaseStatus.Open)
            .GroupBy(c => c.NavigatorId)
            .Select(g => new { NavigatorId = g.Key, Count = g.Count() })
            .ToDictionaryAsync(x => x.NavigatorId, x => x.Count);

        return practitioners.Select(p => new PractitionerResourceDto
        {
            PractitionerId = p.PractitionerId,
            FullName = p.FullName,
            Position = p.Position.ToString(),
            ActiveCases = activeCaseCounts.GetValueOrDefault(p.PractitionerId, 0),
            IsInServiceArea = !string.IsNullOrEmpty(patientZip) && p.ServiceAreas.Any(s => s.ZipCode == patientZip),
    ...
    )
    {
        ...
        var openCases = await context.CareNavigationCases
            .Where(c => c.Status == Domain.Enums.CaseStatus.Open)
            .GroupBy(c => c.NavigatorId)
            .Select(g => new { NavigatorId = g.Key, Count = g.Count() })
            .ToDictionaryAsync(x => x.NavigatorId, x => x.Count);

        var pendingTasks = await context.NavigationTasks
            .Where(t => t.Status == Domain.Enums.NavigationTaskStatus.Pending)
            .GroupBy(t => t.AssignedToId)
            .Select(g => new { NavigatorId = g.Key, Count = g.Count() })
            .ToDictionaryAsync(x => x.NavigatorId, x => x.Count);

        return practitioners.Select(p => {
