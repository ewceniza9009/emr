...
public record UpdateClaimStatusCommand(Guid ClaimId, ClaimStatus NewStatus, string? Remarks = null)
    : IRequest<bool>;
...
public class UpdateClaimStatusCommandHandler : IRequestHandler<UpdateClaimStatusCommand, bool>
{
    ...
    {
        var claim = await _context.ZBenefitClaims.FirstOrDefaultAsync(
            c => c.ClaimId == request.ClaimId,
            cancellationToken
        );
        if (claim == null)
            return false;
        ...
        if (request.NewStatus == ClaimStatus.Approved)
            claim.ApprovedAt = _dateTime.UtcNow;
        if (request.NewStatus == ClaimStatus.Paid)
            claim.PaidAt = _dateTime.UtcNow;
