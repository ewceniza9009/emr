...
public class SearchService : ISearchService
{
    ...
    )
    {
        ...
        var settings = new ConnectionSettings(new Uri(url)).DefaultIndex(PatientIndex);

        _client = new ElasticClient(settings);
    }

    public async Task<List<SearchResultDto>> GlobalSearchAsync(
        string term,
    ...
    )
    {
        ...
        try
        {
            var response = await _client.SearchAsync<ClinicalSearchDocument>(
                s =>
                    s.Index(Indices.Index(PatientIndex).And(OutreachIndex))
                        .Query(q =>
                            q.Bool(b =>
                                b.Must(mu =>
                                        mu.MultiMatch(m =>
            ...
            {
                results = response
                    .Hits.Select(h => new SearchResultDto
                    {
                        Id = Guid.Parse(h.Id),
        ...
        results.AddRange(
            dbPatients.Select(p => new SearchResultDto
            {
                Id = p.PatientId,
                Type = "PATIENT",
        ...
        if (results.Count < 10)
        {
            ...
            results.AddRange(
                dbOutreach.Select(o => new SearchResultDto
                {
    ...
    {
        var primaryPhone =
            patient.Phones?.FirstOrDefault(p => p.IsPrimary)?.PhoneNumber ?? "No Phone";
        var primaryAddr = patient.Addresses?.FirstOrDefault(a => a.IsPrimary)?.Address;
        var location =
            primaryAddr != null ? $"{primaryAddr.City}, {primaryAddr.State}" : "No Address";
        ...
        await _client.IndexAsync(
            doc,
            i => i.Index(PatientIndex).Id(patient.PatientId),
            cancellationToken
        );
    }

    public async Task IndexOutreachAsync(
    ...
    )
    {
        ...
        await _client.IndexAsync(
            doc,
            i => i.Index(OutreachIndex).Id(outreach.PatientOutreachId),
            cancellationToken
        );
    }
    ...
    public async Task RecreateIndicesAsync(CancellationToken cancellationToken)
    {
        ...
        await _client.Indices.CreateAsync(
            PatientIndex,
            c =>
                c.Map<ClinicalSearchDocument>(m =>
                    m.AutoMap().Properties(p => p.Keyword(k => k.Name(n => n.TenantId)))
                ),
            cancellationToken
        );
